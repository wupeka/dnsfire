...
#else  /* if (defined(__BYTE_ORDER) && defined(__LITTLE_ENDIAN) && __BYTE_ORDER
       * == __LITTLE_ENDIAN) || (defined(i386) || defined(__i386__) ||
       * defined(__i486__) || defined(__i586__) || defined(__i686__) ||
       * defined(vax) || defined(MIPSEL)) */
# define HASH_LITTLE_ENDIAN 0
# define HASH_BIG_ENDIAN 0
#endif /* if (defined(__BYTE_ORDER) && defined(__LITTLE_ENDIAN) && __BYTE_ORDER
        * == __LITTLE_ENDIAN) || (defined(i386) || defined(__i386__) ||
        * defined(__i486__) || defined(__i586__) || defined(__i686__) ||
        * defined(vax) || defined(MIPSEL)) */
...
hashlittle( const void *key, size_t length, uint32_t initval)
{
	...
	if (HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0)) {
		 ...
		 *but
		 * then masks off the part it's not allowed to read.  Because
		 *the
		 * string is aligned, the masked-off tail is in the same word as
		 *the
		 * rest of the string.  Every machine with memory protection
		 *I've seen
		 * does it on word boundaries, so is OK with this.  But VALGRIND
		 *will
		 * still catch it and complain.  The masking trick does make the
		 *hash
	...
	uint32_t   *pb)  /* IN: secondary initval, OUT: secondary hash */
{
	...
	if (HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0)) {
		...
		case 2: a += k[0] & 0xffff; break;
		case 1: a += k[0] & 0xff; break;
		case 0: *pc = c; *pb = b; return; /* zero length strings require
			                           * no mixing */
		}
...
hashbig( const void *key, size_t length, uint32_t initval)
{
	...
	if (HASH_BIG_ENDIAN && ((u.i & 0x3) == 0)) {
		 ...
		 *the
		 * string is aligned, the illegal read is in the same word as
		 *the
		 * rest of the string.  Every machine with memory protection
		 *I've seen
		 * does it on word boundaries, so is OK with this.  But VALGRIND
		...
		for (i = 0; i < hlen; ++i) { /*----------------------- for each
			...
			for (j = 0; j < 8; ++j) { /*------------------------ for
				...
				for (m = 1; m < 8; ++m) { /*------------ for
					...
					/*---- check that every output bit is
					 * affected by that input bit */
					for (k = 0; k < MAXPAIR; k += 2) {
						...
						/* have a and b be two keys
						 * differing in only one bit */
						a[i] ^= (k << j);
						a[i] ^= (k >> (8 - j));
						...
						/* check every bit is 1, 0, set,
						 * and not set at least once */
						for (l = 0; l < HASHSTATE;
						     ++l)
						{
							e[l] &= (c[l] ^ d[l]);
...
driver5()
{
	...
	b = 0, c = 0,
	hashlittle2("Four score and seven years ago", 30, &c, &b);
